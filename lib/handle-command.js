...
const {queryAggregateList} = require("./load-aggregate");
...
const handleCommand = async (command) => {
    ...
    if (invokeKey === false) {
        ...
        const aggregateIds = exeFn.makeAggregateIds(invokeKeyObject, invokeKey);
        // ftDev.logJsonString(aggregateIds, `${LN}exeFn.makeAggregateIds(${invokeKey}):`);

        // TODO: Add deeper validation
        if (aggregateIds !== false) {
            let aggregates = await queryAggregateList(aggregateIds); //Promise.all(map(loadAggregateByKey, aggregateIds));
            // ftDev.logJsonString(aggregates, `${LN}aggregates:`);

            // TODO: Add Timeout !!!
            // TODO: add try catch + error logging
            let counter = 0;
            do {
                counter += 1;
                ...
                if (checkMissingAggregate(resultData)) {
                    ...
                    const missingAggregates = await queryAggregateList(resultData.keys); //Promise.all(map(loadAggregateByKey, resultData.keys));
                    // TODO: Add load Error!
                    aggregates = concat(aggregates, missingAggregates);
                }
                resultData = await exeFn.createAggregates(aggregates, invokeKeyObject, invokeKey);
