const {map, split, is, equals, curryN, isNil} = require('ramda');
...
const matchObjectType2 = curryN(2, matchObjectType);
...
const findByType2 = curryN(2, (findType, aggregates) => findByType(aggregates, findType));

const filterByType = (aggregates, filterType) => {
    if (!isArray(aggregates)) {
        aggregates = [aggregates];
    }
    return aggregates.filter(d => matchObjectType(d.key, filterType));
};
 ...
 * @param id
 */
const objectKey2 = curryN(2, (objectType, id) => objectKey(id, objectType));
...
const objectSearch2 = curryN(2, (objectType, index) => objectSearch(index, objectType));
...
const aggregateExists = aggregate => (!isNil(aggregate) && aggregate.exists === true);

class TypeHelper {
    constructor(CONTEXT, AGGREGATE) {
        this.CONTEXT = CONTEXT;
        this.AGGREGATE = AGGREGATE;
    }

    objectType(){
        return objectType(this.CONTEXT, this.AGGREGATE);
    }

    objectKey(id) {
        return objectKey(id, this.objectType());
    }

    invokeKeyString(invokeId) {
        return invokeKeyString(this.CONTEXT, this.AGGREGATE, invokeId);
    }

    matchObjectType(objectType) {
        return matchObjectType(this.objectType(), objectType);
    }

    findByType(aggregates) {
        return findByType(aggregates, this.objectType());
    }

    filterByType(aggregates) {
        return filterByType(aggregates, this.objectType());
    }

    objectSearch(index) {
        return objectSearch(index, this.objectType())
    }
