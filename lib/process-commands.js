...
const processCommands = async (maxProcessCommands = false) => {
    ...
    while(true) {
        ...
        if (commandResult.ok === 1 && commandResult.value !== null) {
            await handleCommand(commandResult.value); //aggregateKey, objectType);
        } else {
            ftDev.log(`${LN}commandResult.value = null  --- NO UNHANDLED COMMAND FOUND --- `);
            return false;
        }
        if (maxProcessCommands !== false && is(Number, maxProcessCommands)) {
            // ftDev.log(`${LN}maxProcessCommands = ${maxProcessCommands} `);
            maxProcessCommands = maxProcessCommands - 1;
